...
public class HttpServer extends BasicNettyServer {
	...
	public HttpServer(final Injector injector, @Named(LISTEN_PORT_NAME) int port) {
		super(HttpServerConnector.builder().port(port).injector(injector).build());
	}
	...
	public HttpServer() {
		super(HttpServerConnector.builder().port(ServerPort.ZoomulusPort(ServerPort.PortNumber.HTTP)).build());
	}
}
